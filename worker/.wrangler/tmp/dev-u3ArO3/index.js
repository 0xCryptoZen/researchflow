...
// .wrangler/tmp/bundle-3sVsDz/checked-fetch.js
...
// .wrangler/tmp/bundle-3sVsDz/strip-cf-connecting-ip-header.js
...
__name(isD1Available, "isD1Available");
async function getUserFromHeader(env, authHeader) {
...
__name(getUserFromHeader, "getUserFromHeader");
...
app.get("/auth/github/callback", async (c) => {
  ...
  try {
    ...
    ).bind(githubUser.id.toString()).first();
    let userId;
...
app.put("/tasks/:id", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    body.relatedPaperId || "",
    body.relatedConferenceId || "",
    body.status === "completed" ? (/* @__PURE__ */ new Date()).toISOString() : null,
    id,
    user.id
  ).run();
...
async function searchIEEE(query, limit) {
  try {
    ...
    return (data.articles || []).map((article) => ({
      ...
      title: article.title,
      authors: article.authors?.map((a) => a.full_name) || [],
...
__name(searchIEEE, "searchIEEE");
async function searchACM(query, limit) {
  try {
    const response = await fetch(
...
async function searchDBLP(query, limit) {
  try {
    const response = await fetch(
      `https://dblp.org/search/publ/api?q=${encodeURIComponent(query)}&h=${limit}&format=json`
    );
    const data = await response.json();
    return (data.result?.hits?.hit || []).map((hit) => {
      const info = hit.info;
      return {
        id: info.id || `dblp_${hit.id}`,
        title: info.title || "",
        authors: info.authors?.author?.map((a) => a.text) || [],
        abstract: "",
        source: "dblp",
        url: info.url || "",
        year: info.year ? parseInt(info.year) : void 0,
        venue: info.venue || "",
...
// .wrangler/tmp/bundle-3sVsDz/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3sVsDz/middleware-loader.entry.ts
